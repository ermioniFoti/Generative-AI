...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        sdf_tensor = data['gt_sdf'].to(device) # Ground truth sdf values 
        # Calculate the loss 
        loss_function = torch.abs(torch.clamp(pred_sdf_tensor,-sigma,sigma) - torch.clamp(sdf_tensor,-sigma,sigma)).sum() # sum() used to accumulate total loss over the batch points
        # Perform backpropagation on the loss
        loss_function.backward() 
        loss_sum += loss_function.item()
        loss_count += xyz_tensor.shape[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_pts", default="data/sphere.pts", type=str, help="Input point cloud")
