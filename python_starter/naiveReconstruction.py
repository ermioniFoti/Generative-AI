...
def naiveReconstruction(points, normals, X, Y, Z):
    ...
    Q = np.array([X.reshape(-1), Y.reshape(-1), Z.reshape(-1)]).transpose()
    tree = KDTree(points)
    _, idx = tree.query(Q, k=1)  # k is the number of nearest neighbors to return so in our case k = 1

    diff = Q - points[idx].squeeze()  # points[idx] has shape (N,1,3) -> squeeze to (N,3) to match Q

    IF = np.sum(normals[idx].squeeze() * diff, axis=1).reshape(X.shape)   #  normals[idx] has shape (N,1,3) -> squeeze to (N,3) to match diff
